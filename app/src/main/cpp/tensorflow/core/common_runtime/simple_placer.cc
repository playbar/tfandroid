...
// (implied by ColocationGraph::ColocateNodes() invocations) are added.
        ...
        public:
            ...
            Status InitializeMember(const Node& node, Member* member) {
                ...
                DCHECK_GE(id, 0);
                member->parent = id;
                TF_RETURN_IF_ERROR(SupportedDeviceTypesForNode(device_types_, node.def(), &member->supported_device_types));

                if(node.has_assigned_device_name() == 0 )
                {
                    int i = 0;
                }
